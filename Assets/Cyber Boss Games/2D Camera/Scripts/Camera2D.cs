...
public class Camera2D : MonoBehaviour
{
    ...
    /// </summary>
    public float followDampeningX = 1f;

    /// <summary>
    /// How fast to track the target along the Y axis.
    /// A higher number means a faster track speed
    /// </summary>
    public float followDampeningY = 1f;
    ...
    private void Look() {
        ...
        if (targets.Count == 1) {
                ...
                Vector3 targetPlayerPos = new Vector3(GetTargetPosition(targets[0].target).x + lookAhead.x, GetTargetPosition(targets[0].target).y + lookAhead.y, transform.position.z);
